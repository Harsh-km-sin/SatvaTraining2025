document.getElementById("preview-btn").addEventListener("click", (event) => {
  ...
  const taxClassification = $("input[name='taxClassification']:checked")
    .map(function () {
      return $(this).val();
    })
    .get()
  ...
  pdf.text(`City: ${city}`, 10, 50);
  pdf.text(`State: ${state}`, 10, 60);
  pdf.text(`ZIP Code: ${zipCode}`, 10, 70);
  pdf.text(`Requester First Name: ${requesterFirstName}`, 10, 80);
  pdf.text(`Requester Last Name: ${requesterLastName}`, 10, 90);
  pdf.text(`Requester Address: ${requesterAddress}`, 10, 100);
  pdf.text(`Requester City: ${requesterCity}`, 10, 110);
  pdf.text(`Requester State: ${requesterState}`, 10, 120);
  pdf.text(`Submission Date: ${submissionDate}`, 10, 130);
  pdf.text(`SSN: ${ssn}`, 10, 140);
  pdf.text(`EIN: ${ein}`, 10, 150);
  pdf.text(`Signature: ${signatureData}`, 10, 160);
...
function validateForm(event) {
  ...
  if (!namePattern.test(entityName)) {
    $("#entityNameError").text(
      "Entity name must contain only letters and spaces."
    );
  ...
  const businessName = $("#businessName").val();
  if (businessName.length == 0 && !namePattern.test(businessName)) {
  ...
  const taxClassificationChecked =
    $("input[name='taxClassification']:checked").length > 0;
  if (!taxClassificationChecked) {
    $("#taxClassificationError").text(
      "Please select at least one tax classification."
    );
    isValid = false;
  } else {
  ...
  if (!addressPattern.test(requesterAddress)) {
    $("#requesterAddressError").text(
      "Requester address can include letters, numbers, spaces, commas, periods, and hyphens."
    );
  ...
  if (!cityStatePattern.test(requesterCity)) {
    $("#requesterCityError").text(
      "Requester city must contain only letters and spaces."
    );
  ...
  if (!cityStatePattern.test(requesterState)) {
    $("#requesterStateError").text(
      "Requester state must contain only letters and spaces."
    );
  ...
  if (!datePattern.test(submissionDate)) {
    $("#submissionDateError").text("Submission date must required.");
  ...
  if (!ssnPattern.test(ssn)) {
    ...
    // $("input[name='ssn1']").addClass("is-invalid");
    isValid = false;
  } else {
    $("input[name='ssn1']").removeClass("is-invalid");
  }
  ...
  if (!einPattern.test(ein)) {
    ...
    // $("input[name='ein1']").addClass("is-invalid");
    isValid = false;
  } else {
    $("input[name='ein1']").removeClass("is-invalid");
  }
  ...
  if (isValid) {
    const signatureData = signaturePad.toDataURL(); // Base64 encoded image
